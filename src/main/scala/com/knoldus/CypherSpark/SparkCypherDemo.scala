...
object SparkCypherDemo extends App {
    ...
    val graph = morpheus.readFrom(personsNode,moviesNode,actedInRelation)


    //Query to get all Person nodes
    val actors = graph.cypher(
        "MATCH (p:Person) return p.name AS Actor_Name"
    )
    actors.records.show

    //Query to get all Movie nodes
    val movies = graph.cypher(
